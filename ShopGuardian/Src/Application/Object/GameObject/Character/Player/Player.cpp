...
void Player::PostUpdate()
{
	...
	//GUI管理者にプレイヤーHPを渡す
	m_spGameScene->GetGameData()->SetPlayerHP(m_hp);
}
...
void Player::Action()
{
	...
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
		if (m_shotCount <= 0)
		{
			...
			bullet->Init();
			bullet->Shot(m_bullet,Bullet::ShotType::Normal,m_pos,angle);
	...
	//ロックオン攻撃
	{
		...
		if (!m_lockFlg && !m_lockEnemy.empty())
		{
			...
			for (auto& ene : m_lockEnemy)
			{
				...
				std::shared_ptr<Bullet> bullet = std::make_shared<Bullet>();
				bullet->Init();
				bullet->Shot(m_bullet,Bullet::ShotType::Lock, m_pos, atan2(dis.y, dis.x));
				bullet->SetGameScene(m_spGameScene);
				m_spGameScene->AddObjList(bullet);
